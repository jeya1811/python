...
class Spawner:
    ...
    def incdiff(self, ball, dt):
        ...
        if (now - self.difftime) > 1000:
            self.difftime = now
            self.speed += 10
            ball.speed += 2.5

    def collision(self, ball):
        return any(ball.collide(b.rect) for b in self.blocks)

    def update(self, surface, ball, dt):
        self.move(dt)
        self.draw(surface)
        self.incdiff(ball, dt)
        self.collision(ball)

    def isspawn(self):
        return not (self.lastblock) or (self.lastblock.rect.y) >= self.h // 4
