import pygame, sys
from player import Ball as player
...
def refresh():
    return player(width, height), Spawner(width, height), Ui(font)
...
while True:
    ...
    if state == State.play:
        ...
        if spawner.collision(ball):
            state = State.out
            pygame.time.set_timer(restart, 2000)
    elif state == State.start:
        ui.startupdate(screen)
    elif state == State.out and hasattr(ball, 'animation'):
        ball.animation(screen)
    elif state == State.reset:
        ui.outupdate(screen)
    ui.fps(screen, clock)
