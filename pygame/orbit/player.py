...
class Ball:
    ...
    def draw(self, surface):
        pygame.draw.circle(surface, self.color, self.ballpos1, self.r, 10)
        pygame.draw.circle(surface, self.color, self.ballpos2, self.r, 10)

    def collidecheck(self, point, rect):
        closestx = max(rect.left, min(point.x, rect.right))
        closesty = max(rect.top, min(point.y, rect.bottom))
        colx = point.x - closestx
        coly = point.y - closesty
        col = (colx * colx) + (coly * coly)
        return col <= (self.r * self.r)

    def collide(self, rect):
        return self.collidecheck(self.ballpos1, rect) or self.collidecheck(self.ballpos2, rect)
...
class Ballsprite(pygame.sprite.Sprite):
    ...
    def collide(self, rect):
        return self.collidecheck(self.rect1, rect) or self.collidecheck(self.rect2, rect)
...
class Box(pygame.sprite.Sprite):
    ...
    def update(self, hold, dt):
        if hold:
            ...
            if mx > self.w // 2:
                self.angle += self.speed * dt
                self.rotoangle += self.rotospeed * dt
            elif mx < self.w // 2:
                self.angle -= self.speed * dt
                self.rotoangle -= self.rotospeed * dt
            else:
